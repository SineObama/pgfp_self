...
<body>
	...
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">关联企业:</label>
			<div class="controls">
				<form:hidden id="articleDataCompany" path="articleData.company" htmlEscape="false" maxlength="200" class="input-xlarge"/>
				<ol id="companyInfoSelectList"></ol>
				<a id="companyButton" href="javascript:" class="btn">添加企业</a>
				<script type="text/javascript">
					...
					function companyInfoSelectRefresh(){
						...
						for (var i=0; i<companyInfoSelect.length; i++){
							...
							$("#articleDataCompany").val($("#articleDataCompany").val()+companyInfoSelect[i][0]+",");
						}
					}
					...
					$("#companyButton").click(function(){
						top.$.jBox.open("iframe:${ctx}/bf/bfCompanyInfo/selectList?pageSize=10", "添加相关",$(top.document).width()-220,$(top.document).height()-180,{
			...
			<div class="controls">
				...
				<script type="text/javascript">
					...
					function articleSelectAddOrDel(id,title){
						...
						top.mainFrame.articleSelect = articleSelect;
					}
				
					top.mainFrame.articleSelectAddOrDel = 	function (id,title){
						var isExtents = false, index = 0;
						for (var i=0; i<articleSelect.length; i++){
							if (articleSelect[i][0]==id){
								isExtents = true;
		...
		<div class="control-group">
			<label class="control-label">是否允许报名:</label>
			<div class="controls">
				<form:radiobuttons path="articleData.allowEnroll" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test="${not empty article.id}">
			...
			<div class="control-group">
				<label class="control-label">查看报名:</label>
				<div class="controls">
					<input id="btnComment" class="btn" type="button" value="查看报名" onclick="viewComment('${ctx}/bf/bfHelpEnroll/?module=bf&contentId=${article.id}&status=0')"/>
					<script type="text/javascript">
						function viewComment(href){
							top.$.jBox.open('iframe:'+href,'查看报名',$(top.document).width()-220,$(top.document).height()-180,{
								buttons:{"关闭":true},
								loaded:function(h){
									$(".jbox-content", top.document).css("overflow-y","hidden");
									$(".nav,.form-actions,[class=btn]", h.find("iframe").contents()).hide();
