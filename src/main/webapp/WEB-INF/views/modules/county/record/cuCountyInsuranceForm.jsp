...
<html>
<head>
	<title>县农村居民购买医疗保险或商业保险情况统计表管理</title>
		...
		$(document).ready(function() {
            ...
            $('.btnEx').click(function () {
                top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v) {
                    var cuTable = null;
                    if (v == "ok") {
                        window.location.href = "${ctx}/record/cuCountyInsurance/export?type=${cuTable.type}&cuMainId=${cuTable.cuMainId}";
                    }
                });
            });

            $(function () {
                $(".content-nav #insurance").attr("class", "active");
            });
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		...
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyInsurance/save" method="post"
			...
			<div>
				<table>
					...
					<tr>
						<td colspan="2">购买医疗保险（含大病保险）</td>
						<td colspan="2">购买商业保险</td>
					</tr>
					<tr>
						<td>户数</td>
						<td>购买率</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyInsurances}">
						<c:forEach items="${cuTable.cuCountyInsurances}" var="county" varStatus="status">
							<c:set var="indexN" value="${status.index+1}" scope="page"/>
							<input type="hidden" class="id" name="cuCountyInsurances[${status.index}].id"
								   value="${county.id}"/>
							<tr>
								<td class="bg-white">${indexN}</td>
								<td class="bg-white required town">
									<input type="hidden" class="townId"
										   name="cuCountyInsurances[${status.index}].town.id"
										   value="${county.town.id}"/>
									<input class="td-input townName required"
										   name="cuCountyInsurances[${status.index}].town.name"
										   readonly="true" value="${county.town.name}"/>
								</td>
								<td class="bg-white village">
									<input type="hidden" class="villageId"
										   name="cuCountyInsurances[${status.index}].village.id"
										   value="${county.village.id}"/>
									<input class="td-input villageName required"
										   name="cuCountyInsurances[${status.index}].village.name"
										   readonly="true" value="${county.village.name}"/>
								</td>
								<td class="bg-white">
									<input class="farmersNumber int td-input required"
										   name="cuCountyInsurances[${status.index}].farmersNumber"
										   value="${county.farmersNumber}" maxlength="8"/>
								</td>
								<td class="bg-white">
									<input class="insuranceNumber int td-input required"
										   name="cuCountyInsurances[${status.index}].insuranceNumber"
										   value="${county.insuranceNumber}" maxlength="8"/>
								</td>
								<td style="color: black;">
									${county.insuranceRate}
								</td>
								<td class="bg-white">
									<input class="medicareNumber td-input int required"
										   name="cuCountyInsurances[${status.index}].medicareNumber"
										   value="${county.medicareNumber}" maxlength="8"/>
								</td>
								<td style="color: black;">
									${county.medicareRate}
								</td>
								<td class="bg-white">
									<input class="businessNumber td-input int required"
										   name="cuCountyInsurances[${status.index}].businessNumber"
										   value="${county.businessNumber}" maxlength="8"/>
								</td>
								<td style="color: black;">
									${county.businessRate}
								</td>
								<td class="bg-white">
									<input class="remarks td-input"
										   name="cuCountyInsurances[${status.index}].remarks"
			...
			<div class="form-actions actions">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountyInsurance:edit">
...
<script>
    ...
    $(function () {
        $(".add-tr-btn").click(function () {
            ...
            var num = index + renum;
            var tr = '<tr>' +
                '<input type="hidden" class="id"  name="cuCountyInsurances[' + num + '].id"/>' +
                '<td class="bg-white">' +
                '</td>' +
                '<td class="bg-white town">' +
                '<input type="hidden" class="townId" name="cuCountyInsurances[' + num + '].town.id"/>' +
                '<input class="td-input townName required" readonly="true" name="cuCountyInsurances[' + num + '].town.name"/>' +
                '</td>' +
                ' <td class="bg-white village">' +
                '<input type="hidden" class="villageId" name="cuCountyInsurances[' + num + '].village.id"/>' +
                '<input class="td-input villageName required" readonly="true" name="cuCountyInsurances[' + num + '].village.name"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input farmersNumber int required" maxlength="8" name="cuCountyInsurances[' + num + '].farmersNumber"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input insuranceNumber int required" maxlength="8" name="cuCountyInsurances[' + num + '].insuranceNumber"/> ' +
                '</td>' +
                ' <td>' +
                // '<input class="td-input insuranceRate" name="cuCountyInsurances[' + num + '].insuranceRate" readonly/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input medicareNumber int required" maxlength="8" name="cuCountyInsurances[' + num + '].medicareNumber"/> ' +
                '</td>' +
                ' <td>' +
                // '<input class="td-input medicareRate" name="cuCountyInsurances[' + num + '].medicareRate" readonly/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input businessNumber int required" maxlength="8" name="cuCountyInsurances[' + num + '].businessNumber"/> ' +
                '</td>' +
                ' <td>' +
                // '<input class="businessRate td-input" name="cuCountyInsurances[' + num + '].businessRate" readonly/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input remarks" name="cuCountyInsurances[' + num + '].remarks" maxlength="500"/>' +
                '</td>' +
                '<td class="bg-white" style="text-align: center">' +
                '<span class="del-thistd-btn">删除</span>' +
                '</td>' +
