...
<head>
	...
	<script type="text/javascript">
            ...
            var tr ="<tr>"
                ...
                <%--乡镇--%>
                	+ "<td class='bg-white town'>"
                		+ "<input type='hidden'name='cuCountyHouseUnqualifieds[" + index + "].town.id' class='townId'/>"
                		+ "<input class='input_none townName required'style='background-color: #FFFFFF'readonly='true'/>"
                	+ "</td>"
                <%--村--%>
                	+ "<td class='bg-white country'>"
                		+ "<input type='hidden' name='cuCountyHouseUnqualifieds[" + index + "].country.id' class='countryId'/>"
                		+ "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
                	+ "</td>"
                <%--屯--%>
                	+ "<td class='bg-white tun'>"
						+ "<input type='hidden' name='cuCountyHouseUnqualifieds[" + index + "].tun.id' class='tunId'/>"
                		+ "<input class='input_none required tunName' style='background-color: #FFFFFF' readonly='true'/>"
                	+ "</td>"
                <%--户主姓名--%>
                	+ "<td class='bg-white'>"
                		+ "<input name='cuCountyHouseUnqualifieds[" + index + "].householdName' class='input_none required realName'/>"
                	+ "</td>"
                <%--家庭人口数--%>
                	+ "<td class='bg-white'>"
						+ "<input name='cuCountyHouseUnqualifieds[" + index + "].familyMembers'class='input_none required int'"
						+ "maxlength='4'/>"
                	+ "</td>"
                <%--未达标原因--%>
                	+ "<td class='bg-white'>"
						+ "<input name='cuCountyHouseUnqualifieds[" + index + "].notQualifiedReason'class='input_none'/>"
                	+ "</td>"
                <%--解决措施--%>
                	+ "<td class='bg-white'>"
						+ "<input name='cuCountyHouseUnqualifieds[" + index + "].solution'class='input_none'/>"
                	+ "</td>"
                <%--备注--%>
                	+ "<td class='bg-white'>"
						+ "<input name='cuCountyHouseUnqualifieds[" + index + "].remarks'class='input_none'/>"
                	+ "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #HouseUnqualified").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyHouseUnqualified/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县住房保障未达标户名单列表</h1>
			<br/><br/>
			<p>
				...
				<span class="county">填报单位：
					...
					<input name="department.name" class="underline required countyName" value="${cuTable.department.name}"/>县(市、区）住房城乡建设部门（盖章）
				</span>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="60px">户主姓名</td>
						<td width="40px">家庭人口数</td>
						<td width="180px">未达标原因</td>
						<td width="180px">解决措施</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyHouseUnqualifieds}">
						<c:forEach items="${cuTable.cuCountyHouseUnqualifieds}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyHouseUnqualifieds[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								...
								<td class="bg-white town">
									<input type="hidden"
										   name="cuCountyHouseUnqualifieds[${index}].town.id"
										   class="townId"
										   value="${house.town.id}"/>
									...
									<%--村--%>
								<td class="bg-white country">
									<input type="hidden" name="cuCountyHouseUnqualifieds[${index}].country.id"
										   class="countryId "
									...
									<%--屯--%>
								<td class="bg-white tun">
									<input type="hidden" name="cuCountyHouseUnqualifieds[${index}].tun.id"
										   class="tunId "
										   value="${house.tun.id}"/>
									<input class="input_none required tunName required"
										   style="background-color: #FFFFFF"
										   readonly="true"
									...
									<%--户主姓名--%>
								<td class="bg-white">
									<input name="cuCountyHouseUnqualifieds[${index}].householdName"
										   class="input_none required realName"
										   value="${house.householdName}"/>
								</td>
									<%--家庭人口数--%>
								<td class="bg-white">
									<input name="cuCountyHouseUnqualifieds[${index}].familyMembers"
										   class="input_none required int"
										   maxlength="4"
										   value="${house.familyMembers}"/>
								</td>
									<%--未达标原因--%>
								<td class="bg-white">
									<input name="cuCountyHouseUnqualifieds[${index}].notQualifiedReason"
										   class="input_none"
										   value="${house.notQualifiedReason}"/>
								</td>
									<%--解决措施--%>
								<td class="bg-white">
									<input name="cuCountyHouseUnqualifieds[${index}].solution"
										   class="input_none"
										   value="${house.solution}"/>
								</td>
									<%--备注--%>
								<td class="bg-white">
									<input name="cuCountyHouseUnqualifieds[${index}].remarks"
										   class="input_none"
										   value="${house.remarks}"/>
								</td>
			...
			<div class="form-actions" style="padding-left: 20px!important;">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountyHouseUnqualified:edit">
