...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		...
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyIndustrialBase/save" method="post"
			...
			<p style="margin-bottom: 5px!important;">
             ...
             填报单位：<input class="underline departmentName required" readonly="true" name="department.name"
						 value="平桂"
						 style="width: 80px" maxlength="20"/>县（市、区）产业开发专责小组（盖章）
                </span>
			...
			<div>
				<table>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyIndustrialBases}">
						<c:forEach items="${cuTable.cuCountyIndustrialBases}" var="base" varStatus="status">
							<c:set var="indexN" value="${status.index+1}" scope="page"/>
							<input type="hidden" class="id" name="cuCountyIndustrialBases[${status.index}].id"
								   value="${base.id}"/>
							<tr class="tr-1">
								<td class="bg-white">${indexN}</td>
								<td class="bg-white required town">
									<input type="hidden" class="townId"
										   name="cuCountyIndustrialBases[${status.index}].town.id"
										   value="${base.town.id}"/>
									<input class="td-input townName required"
										   name="cuCountyIndustrialBases[${status.index}].town.name"
										   readonly="true" value="${base.town.name}"/>
								</td>
								<td class="bg-white village">
									<input type="hidden" class="villageId"
										   name="cuCountyIndustrialBases[${status.index}].village.id"
										   value="${base.village.id}"/>
									<input class="td-input villageName required"
										   name="cuCountyIndustrialBases[${status.index}].village.name"
										   readonly="true" value="${base.village.name}"/>
								</td>
								<td class="bg-white">
									<input class="industrialBaseName td-input required"
										   name="cuCountyIndustrialBases[${status.index}].industrialBaseName"
										   value="${base.industrialBaseName}" maxlength="30"/>
								</td>
								<td class="bg-white">
									<input class="industrialBaseScale td-input required"
										   name="cuCountyIndustrialBases[${status.index}].industrialBaseScale"
										   value="${base.industrialBaseScale}" maxlength="30"/>
								</td>
								<td class="bg-white">
									<input class="coveragePoorHouse td-input int required"
										   name="cuCountyIndustrialBases[${status.index}].coveragePoorHouse"
										   value="${base.coveragePoorHouse}" maxlength="8"/>
								</td>
								<td class="bg-white">
									<input class="remarks td-input"
										   name="cuCountyIndustrialBases[${status.index}].remarks"
										   value="${base.remarks}"/>
								</td>
			...
			<div class="form-actions actions">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountyIndustrialBase:edit">
...
<script>
    ...
    $(function () {
        $(".add-tr-btn").click(function () {
            ...
            var num = index + renum;
            var tr = '<tr>' +
                '<input type="hidden" class="id"  name="cuCountyIndustrialBases[' + num + '].id"/>' +
                '<td class="bg-white">' +
                '</td>' +
                '<td class="bg-white town">' +
                '<input type="hidden" class="townId" name="cuCountyIndustrialBases[' + num + '].town.id"/>' +
                '<input class="td-input townName required" readonly="true" name="cuCountyIndustrialBases[' +num + '].town.name"/>' +
                '</td>' +
                ' <td class="bg-white village">' +
                '<input type="hidden" class="villageId" name="cuCountyIndustrialBases[' + num + '].village.id"/>' +
                '<input class="td-input villageName required" readonly="true" name="cuCountyIndustrialBases[' +num + '].village.name"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input industrialBaseName required" maxlength="64" name="cuCountyIndustrialBases[' + num + '].industrialBaseName"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input industrialBaseScale required" maxlength="30" name="cuCountyIndustrialBases[' + num + '].industrialBaseScale"/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input coveragePoorHouse int required" maxlength="8" name="cuCountyIndustrialBases[' + num + '].coveragePoorHouse"/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input remarks" name="cuCountyIndustrialBases[' + num + '].remarks" maxlength="500"/>' +
                '</td>' +
                '<td class="bg-white" style="text-align: center">' +
                '<span class="del-thistd-btn">删除</span>' +
                '</td>' +
