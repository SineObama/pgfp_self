...
<head>
	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
					...
					+"<td class='bg-white town'>"
						+"<input type='hidden' name='cuCountyDilapidatedHouses[" + index + "].town.id' class='townId'/>"
						+"<input class='input_none townName required'style='background-color: #FFFFFF'readonly='true'/>"
					+"</td>"
					<%--村--%>
					+"<td class='bg-white country'>"
						+"<input type='hidden' name='cuCountyDilapidatedHouses[" + index + "].country.id' class='countryId'/>"
						+"<input class='input_none required countryName' style='background-color: #FFFFFF'readonly='true'/>"
					+"</td>"
					<%--2015年底存量危房户数--%>
					+"<td class='bg-white'>"
						+"<input name='cuCountyDilapidatedHouses[" + index + "].dilapidatedHouses2015' class='input_none required int'/>"
					+"</td>"
					<%--当年存量危房户数--%>
					+"<td class='bg-white'>"
						+"<input name='cuCountyDilapidatedHouses[" + index + "].dilapidatedHouses' class='input_none required int' maxlength='8'/>"
					+"</td>"
					<%--存量危房改造率--%>
					+"<td class='bg-white'>"
						+"<input name='cuCountyDilapidatedHouses[" + index + "].rate' class='input_none'/>"
					+"</td>"
					<%--备注--%>
					+"<td class='bg-white'>"
						+"<input name='cuCountyDilapidatedHouses[" + index + "].remarks' class='input_none'/>"
					+"</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #Dilapidated").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyDilapidatedHouses/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县（市、区）贫困户存量危房改造情况统计表</h1>
			<br/><br/>
			<p>
			...
			<div>
				<table>
					...
					<tr>
						<td width="20px">序号</td>
						<td width="70px">乡（镇）</td>
						<td width="80px">行政村</td>
						<td width="100px">2015年底存量危房户数</td>
						<td width="100px">当年存量危房户数</td>
						<td width="100px">存量危房改造率</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyDilapidatedHouses}">
						<c:forEach items="${cuTable.cuCountyDilapidatedHouses}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyDilapidatedHouses[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								<td class="bg-white">${index}</td>
									<%--乡镇--%>
								<td class="bg-white town">
									<input type="hidden"
										   name="cuCountyDilapidatedHouses[${index}].town.id"
										   class="townId"
										   value="${house.town.id}"/>
									...
									<%--村--%>
								<td class="bg-white country">
									<input type="hidden" name="cuCountyDilapidatedHouses[${index}].country.id"
										   class="countryId "
									...
									<%--2015年底存量危房户数--%>
								<td class="bg-white">
									<input name="cuCountyDilapidatedHouses[${index}].dilapidatedHouses2015"
										   class="input_none required int"
										   value="${house.dilapidatedHouses2015}"/>
								</td>
									<%--当年存量危房户数--%>
								<td class="bg-white">
									<input name="cuCountyDilapidatedHouses[${index}].dilapidatedHouses"
										   class="input_none required int"
										   maxlength="8"
										   value="${house.dilapidatedHouses}"/>
								</td>
									<%--存量危房改造率--%>
								<td class="bg-white">
									<input name="cuCountyDilapidatedHouses[${index}].rate"
										   class="input_none required"
										   value="${house.rate}"/>
								</td>
									<%--备注--%>
								<td class="bg-white">
									<input name="cuCountyDilapidatedHouses[${index}].remarks"
										   class="input_none"
										   value="${house.remarks}"/>
								</td>
								<td class="bg-white" style="text-align: center">
