    ...
    <script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
                ...
                + "<input type='hidden' name='cuCountyCureSubsidies[" + index + "].town.id' class='townId'/>"
                + "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--村--%>
                + "<td class='bg-white country'>"
                + "<input type='hidden' name='cuCountyCureSubsidies[" + index + "].country.id' class='countryId'/>"
                + "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--患病贫困户数--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyCureSubsidies[" + index + "].sickHouseNumber' class='input_none required int' maxlength='8'/>"
                + "</td>"
                <%--患病人数--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyCureSubsidies[" + index + "].sickPoorNumber' class='input_none int' maxlength='8'/>"
                + "</td>"
                <%--得到有效救治人数--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyCureSubsidies[" + index + "].effectiveTreatmentNumber' class='input_none int' maxlength='8'/>"
                + "</td>"
                <%--得到补助报销人数--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyCureSubsidies[" + index + "].subsidyNumber' class='input_none int' maxlength='8'/>"
                + "</td>"
                <%--备注--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyCureSubsidies[" + index + "].remarks' class='input_none'/>"
                + "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #CureSubsidy").attr("class", "active");
        })
...
<div class="content tabs f-clear">
    ...
    <div id="tab-1" class="content-detail">
        <form:form id="inputForm" modelAttribute="cuTable"
                   action="${ctx}/record/cuCountyCureSubsidy/save" method="post" class="form-horizontal">
            ...
            <div>
                <table>
                    ...
                    <tr>
                        ...
                        <td width="60px">患病贫困户数</td>
                        <td width="60ox">患病人数</td>
                        <td width="60px">得到有效救治人数</td>
                        <td width="60px">得到补助报销人数</td>
                    ...
                    <tbody id="accpitem">
                    <c:if test="${!empty cuTable.cuCountyCureSubsidies}">
                        <c:forEach items="${cuTable.cuCountyCureSubsidies}" var="house" varStatus="status">
                            <c:set var="index" value="${status.index}"/>
                            <input type="hidden" name="cuCountyCureSubsidies[${index}].id" value="${house.id}"/>
                            <tr>
                                    <%--序号--%>
                                <td class="bg-white">${index}</td>
                                    <%--乡镇--%>
                                <td class="bg-white town">
                                    <input type="hidden"
                                           name="cuCountyCureSubsidies[${index}].town.id"
                                           class="townId"
                                           value="${house.town.id}"/>
                                    ...
                                    <%--村--%>
                                <td class="bg-white country">
                                    <input type="hidden" name="cuCountyCureSubsidies[${index}].country.id"
                                           class="countryId "
                                    ...
                                    <%--患病贫困户数--%>
                                <td class="bg-white">
                                    <input name="cuCountyCureSubsidies[${index}].sickHouseNumber"
                                           class="input_none required int"
                                           value="${house.sickHouseNumber}"/>
                                </td>
                                    <%--患病人数--%>
                                <td class="bg-white">
                                    <input name="cuCountyCureSubsidies[${index}].sickPoorNumber"
                                           class="input_none int"
                                           maxlength="8"
                                           value="${house.sickPoorNumber}"/>
                                </td>
                                    <%--得到有效救治人数--%>
                                <td class="bg-white">
                                    <input name="cuCountyCureSubsidies[${index}].effectiveTreatmentNumber"
                                           class="input_none int"
                                           maxlength="8"
                                           value="${house.effectiveTreatmentNumber}"/>
                                </td>
                                    <%--得到补助报销人数--%>
                                <td class="bg-white">
                                    <input name="cuCountyCureSubsidies[${index}].subsidyNumber"
                                           class="input_none int"
                                           maxlength="8"
                                           value="${house.subsidyNumber}"/>
                                </td>
                                    <%--备注--%>
                                <td class="bg-white">
                                    <input name="cuCountyCureSubsidies[${index}].remarks"
                                           class="input_none"
                                           value="${house.remarks}"/>
                                </td>
                                <td class="bg-white" style="text-align: center">
