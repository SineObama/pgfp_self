...
<head>
	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
						...
						+ "<td class='bg-white town'>"
							+ "<input type='hidden' name='cuCountyNoSafeWaters[" + index + "].town.id' class='townId'/>"
							+ "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
						+ "</td>"
						<%--村--%>
						+ "<td class='bg-white country'>"
							+ "<input type='hidden' name='cuCountyNoSafeWaters[" + index + "].country.id' class='countryId'/>"
							+ "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
						+ "</td>"

						<%--自然村--%>
						+ "<td class='bg-white tun'>"
							+ "<input type='hidden' name='cuCountyNoSafeWaters[" + index + "].tun.id' class='tunId'/>"
							+ "<input class='input_none required tunName' style='background-color: #FFFFFF' readonly='true'/>"
						+ "</td>"
						<%--未达到安全饮水户主姓名--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountyNoSafeWaters[" + index + "].householdName' class='input_none realName required'/>"
						+ "</td>"
						<%--家庭人口数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountyNoSafeWaters[" + index + "].familyMember' class='input_none int required'/>"
						+ "</td>"
						<%--未达到安全饮水原因--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountyNoSafeWaters[" + index + "].reason' class='input_none'/>"
						+ "</td>"
						<%--解决措施--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountyNoSafeWaters[" + index + "].solution' class='input_none'/>"
						+ "</td>"
						<%--备注--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountyNoSafeWaters[" + index + "].remarks' class='input_none'/>"
						+ "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #NoSafeWater").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyNoSafeWater/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县（市、区）未达到安全饮水户名单</h1>
			<br/><br/>
			<p>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="60px">家庭人口</td>
						<td width="160px">未达标原因</td>
						<td width="180px">解决措施</td>
						<td width="80px" rowspan="2">备注</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyNoSafeWaters}">
						<c:forEach items="${cuTable.cuCountyNoSafeWaters}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyNoSafeWaters[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								...
								<td class='bg-white town'>
									<input type='hidden'
										   name='cuCountyNoSafeWaters[${index}].town.id'
										   class='townId'
										   value='${house.town.id}'/>
									...
									<%--村--%>
								<td class='bg-white country'>
									<input type='hidden' name='cuCountyNoSafeWaters[${index}].country.id'
										   class='countryId '
									...
									<%--自然村--%>
								<td class='bg-white tun'>
									<input type='hidden' name='cuCountyNoSafeWaters[${index}].tun.id'
										   class='tunId '
										   value='${house.tun.id}'/>
									...
									<%--未达到安全饮水户主姓名--%>
								<td class='bg-white'>
									<input name='cuCountyNoSafeWaters[${index}].householdName'
										   class='input_none realName required'
										   value='${house.householdName}'/>
								</td>
									<%--家庭人口数--%>
								<td class='bg-white'>
									<input name='cuCountyNoSafeWaters[${index}].familyMember'
										   class='input_none int required'
										   value='${house.familyMember}'/>
								</td>
									<%--未达到安全饮水原因--%>
								<td class='bg-white'>
									<input name='cuCountyNoSafeWaters[${index}].reason'
										   class='input_none'
										   value='${house.reason}'/>
								</td>
									<%--解决措施--%>
								<td class='bg-white'>
									<input name='cuCountyNoSafeWaters[${index}].solution'
										   class='input_none'
										   value='${house.solution}'/>
								</td>
									<%--备注--%>
								<td class='bg-white'>
									<input name='cuCountyNoSafeWaters[${index}].remarks'
										   class='input_none'
										   value='${house.remarks}'/>
								</td>
								<td class='bg-white' style='text-align: center'>
			...
			<div class="form-actions" style="padding-left: 20px!important;">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountyNoSafeWater:edit">
