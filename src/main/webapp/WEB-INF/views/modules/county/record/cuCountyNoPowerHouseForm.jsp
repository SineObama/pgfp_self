	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
                ...
                + "<input type='hidden' name='cuCountyNoPowerHouses[" + index + "].town.id' class='townId'/>"
                + "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--村--%>
                + "<td class='bg-white country'>"
                + "<input type='hidden' name='cuCountyNoPowerHouses[" + index + "].country.id' class='countryId'/>"
                + "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"

                <%--自然村--%>
                + "<td class='bg-white tun'>"
                + "<input type='hidden' name='cuCountyNoPowerHouses[" + index + "].tun.id' class='tunId'/>"
                + "<input class='input_none required tunName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--户主姓名--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyNoPowerHouses[" + index + "].householdName' class='input_none realName required'/>"
                + "</td>"
                <%--家庭人口数--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyNoPowerHouses[" + index + "].familyMember' class='input_none int' maxlength='2'/>"
                + "</td>"
                <%--未接通原因--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyNoPowerHouses[" + index + "].reason' class='input_none'/>"
                + "</td>"
                <%--备注--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyNoPowerHouses[" + index + "].remarks' class='input_none'/>"
                + "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #NoPowerHouse").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyNoPowerHouse/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县（市、区）未接通生活用电户名单</h1>
			<br/><br/>
			<p>
			...
			<div>
				<table>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyNoPowerHouses}">
						<c:forEach items="${cuTable.cuCountyNoPowerHouses}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyNoPowerHouses[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								...
								<td class='bg-white town'>
									<input type='hidden'
										   name='cuCountyNoPowerHouses[${index}].town.id'
										   class='townId'
										   value='${house.town.id}'/>
									...
									<%--村--%>
								<td class='bg-white country'>
									<input type='hidden' name='cuCountyNoPowerHouses[${index}].country.id'
										   class='countryId '
									...
									<%--自然村--%>
								<td class='bg-white tun'>
									<input type='hidden' name='cuCountyNoPowerHouses[${index}].tun.id'
										   class='tunId '
										   value='${house.tun.id}'/>
									...
									<%--户主姓名--%>
								<td class='bg-white'>
									<input name='cuCountyNoPowerHouses[${index}].householdName'
										   class='input_none realName required'
										   value='${house.householdName}'/>
								</td>
									<%--家庭人口数--%>
								<td class='bg-white'>
									<input name='cuCountyNoPowerHouses[${index}].familyMember'
									...
									<%--未接通原因--%>
								<td class='bg-white'>
									<input name='cuCountyNoPowerHouses[${index}].reason'
										   class='input_none'
										   value='${house.reason}'/>
								</td>
									<%--备注--%>
								<td class='bg-white'>
									<input name='cuCountyNoPowerHouses[${index}].remarks'
										   class='input_none'
										   value='${house.remarks}'/>
								</td>
								<td class='bg-white' style='text-align: center'>
			...
			<div class="form-actions" style="padding-left: 20px!important;">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountyNoPowerHouse:edit">
