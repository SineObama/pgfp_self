        ...
        $(document).ready(function() {
            ...
            $('.btnEx').click(function () {
                top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v) {
                    var cuTable = null;
                    if (v == "ok") {
                        window.location.href = "${ctx}/record/cuCountySickPoorer/export?type=${cuTable.type}&cuMainId=${cuTable.cuMainId}";
                    }
                });
            });

            $(function () {
                $(".content-nav #sick").attr("class", "active");
            });
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		...
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountySickPoorer/save" method="post"
			...
			<p style="margin-bottom: 5px!important;">
             ...
             填报单位：<input class="underline departmentName required" readonly="true" name="department.name"
						 value="平桂"
						 style="width: 80px" maxlength="20"/>县(市、区）卫生计生局
					<input class="underline departmentName required" readonly="true" name="department.name"
						   value="平桂"
						   style="width: 80px" maxlength="20"/>县(市、区）扶贫办（盖章）
                </span>
			...
			<div>
				<table>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountySickPoorers}">
						<c:forEach items="${cuTable.cuCountySickPoorers}" var="county" varStatus="status">
							<c:set var="indexN" value="${status.index+1}" scope="page"/>
							<input type="hidden" class="id" name="cuCountySickPoorers[${status.index}].id"
								   value="${county.id}"/>
							<tr>
								<td class="bg-white">${indexN}</td>
								<td class="bg-white required town">
									<input type="hidden" class="townId"
										   name="cuCountySickPoorers[${status.index}].town.id"
										   value="${county.town.id}"/>
									<input class="td-input townName required"
										   name="cuCountySickPoorers[${status.index}].town.name"
										   readonly="true" value="${county.town.name}"/>
								</td>
								<td class="bg-white village">
									<input type="hidden" class="villageId"
										   name="cuCountySickPoorers[${status.index}].village.id"
										   value="${county.village.id}"/>
									<input class="td-input villageName required"
										   name="cuCountySickPoorers[${status.index}].village.name"
										   readonly="true" value="${county.village.name}"/>
								</td>
								<td class="bg-white">
									<input class="sickPoorHouse int td-input required"
										   name="cuCountySickPoorers[${status.index}].sickPoorHouse"
										   value="${county.sickPoorHouse}" maxlength="8"/>
								</td>
								<td class="bg-white">
									<input class="sickReducePoorHouse int td-input required"
										   name="cuCountySickPoorers[${status.index}].sickReducePoorHouse"
										   value="${county.sickReducePoorHouse}" maxlength="8"/>
								</td>
								...
								<td class="bg-white">
									<input class="remarks td-input"
										   name="cuCountySickPoorers[${status.index}].remarks"
			...
			<div class="form-actions actions">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountySickPoorer:edit">
...
<script>
    ...
    $(function () {
        $(".add-tr-btn").click(function () {
            ...
            var num = index + renum;
            var tr = '<tr>' +
                '<input type="hidden" class="id"  name="cuCountySickPoorers[' + num + '].id"/>' +
                '<td class="bg-white">' +
                '</td>' +
                '<td class="bg-white town">' +
                '<input type="hidden" class="townId" name="cuCountySickPoorers[' + num + '].town.id"/>' +
                '<input class="td-input townName required" readonly="true" name="cuCountySickPoorers[' + num + '].town.name"/>' +
                '</td>' +
                ' <td class="bg-white village">' +
                '<input type="hidden" class="villageId" name="cuCountySickPoorers[' + num + '].village.id"/>' +
                '<input class="td-input villageName required" readonly="true" name="cuCountySickPoorers[' + num + '].village.name"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input sickPoorHouse int required" maxlength="8" name="cuCountySickPoorers[' + num + '].sickPoorHouse"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input sickReducePoorHouse int required" maxlength="8" name="cuCountySickPoorers[' + num + '].sickReducePoorHouse"/> ' +
                '</td>' +
                ' <td>' +
                ' <td class="bg-white">' +
                '<input class="td-input remarks" name="cuCountySickPoorers[' + num + '].remarks" maxlength="500"/>' +
                '</td>' +
                '<td class="bg-white" style="text-align: center">' +
                '<span class="del-thistd-btn">删除</span>' +
                '</td>' +
