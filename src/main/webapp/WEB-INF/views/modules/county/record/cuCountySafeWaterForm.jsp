...
<head>
	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
						...
						+ "<td class='bg-white town'>"
							+ "<input type='hidden' name='cuCountySafeWaters[" + index + "].town.id' class='townId'/>"
							+ "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
						+ "</td>"
						<%--村--%>
						+ "<td class='bg-white country'>"
							+ "<input type='hidden' name='cuCountySafeWaters[" + index + "].country.id' class='countryId'/>"
							+ "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
						+ "</td>"
						<%--户数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].houseNumber' class='input_none int required' maxlength='8'/>"
						+ "</td>"
						<%--有安全饮水户数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].safeWaterHouse' class='input_none int' maxlength='8'/>"
						+ "</td>"
						<%--打井户数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].wellHouse' class='input_none int' maxlength='8'/>"
						+ "</td>"
						<%--建水柜户数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].waterTankHouse' class='input_none int' maxlength='8'/>"
						+ "</td>"
						<%--建水窖户数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].cisternHouse' class='input_none int' maxlength='8'/>"
						+ "</td>"
						<%--引用山泉水--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].springWaterHouse' class='input_none int' maxlength='8'/>"
						+ "</td>"
						<%--自来水户数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].tapWaterHouse' class='input_none int' maxlength='8'/>"
						+ "</td>"
						<%--未达到安全饮水户数--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].noSafeWaterHouse' class='input_none int' maxlength='8'/>"
						+ "</td>"
                        <%--安全饮水达标率--%>
                        + "<td class='bg-white'>"
                            + "<input name='cuCountySafeWaters[" + index + "].rate' class='input_none decimal'/>"
                        + "</td>"
						<%--备注--%>
						+ "<td class='bg-white'>"
							+ "<input name='cuCountySafeWaters[" + index + "].remarks' class='input_none'/>"
						+ "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #SafeWater").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountySafeWater/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县（市、区）有安全饮水达标情况统计表</h1>
			<br/><br/>
			<p>
				...
				<span class="county">填报单位：
					...
					<input name="department.name" class="underline required countyName"
						   value="${cuTable.department.name}"/>县(市、区）水利局（盖章）
				</span>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="60px" rowspan="2">户数</td>
						<td width="55px" rowspan="2">达标户数</td>
						<td colspan="5">其中</td>
						<td width="260px" rowspan="2">未达标户数</td>
						<td width="20px" rowspan="2">达标率</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountySafeWaters}">
						<c:forEach items="${cuTable.cuCountySafeWaters}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountySafeWaters[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								...
								<td class='bg-white town'>
									<input type='hidden'
										   name='cuCountySafeWaters[${index}].town.id'
										   class='townId'
										   value='${house.town.id}'/>
									...
									<%--村--%>
								<td class='bg-white country'>
									<input type='hidden' name='cuCountySafeWaters[${index}].country.id'
										   class='countryId '
									...
									<%--户数--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].houseNumber'
									...
									<%--有安全饮水户数--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].safeWaterHouse'
										   class='input_none int'
										   maxlength='8'
										   value='${house.safeWaterHouse}'/>
								</td>
									<%--打井户数--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].wellHouse'
										   class='input_none int'
										   maxlength='8'
										   value='${house.wellHouse}'/>
								</td>
									<%--建水柜户数--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].waterTankHouse'
										   class='input_none int'
										   maxlength='8'
										   value='${house.waterTankHouse}'/>
								</td>
									<%--建水窖户数--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].cisternHouse'
										   class='input_none int'
										   maxlength='8'
										   value='${house.cisternHouse}'/>
								</td>
									<%--引用山泉水--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].springWaterHouse'
										   class='input_none int'
										   maxlength='8'
										   value='${house.springWaterHouse}'/>
								</td>
									<%--自来水户数--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].tapWaterHouse'
										   class='input_none int'
										   maxlength='8'
										   value='${house.tapWaterHouse}'/>
								</td>
									<%--未达到安全饮水户数--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].noSafeWaterHouse'
										   class='input_none int'
										   maxlength='8'
										   value='${house.noSafeWaterHouse}'/>
								</td>
                                    <%--安全饮水达标率--%>
                                <td class='bg-white'>
                                    <input name='cuCountySafeWaters[${index}].rate'
                                           class='input_none decimal'
                                           value='${house.rate}'/>
                                </td>
									<%--备注--%>
								<td class='bg-white'>
									<input name='cuCountySafeWaters[${index}].remarks'
										   class='input_none'
										   value='${house.remarks}'/>
								</td>
								<td class='bg-white' style='text-align: center'>
			...
			<div class="form-actions" style="padding-left: 20px!important;">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountySafeWater:edit">
