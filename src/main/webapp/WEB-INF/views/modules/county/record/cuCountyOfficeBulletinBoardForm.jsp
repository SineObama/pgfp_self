	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
                ...
                + "<input type='hidden' name='cuCountyOfficeBulletinBoards[" + index + "].town.id' class='townId'/>"
                + "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--村--%>
                + "<td class='bg-white country'>"
                + "<input type='hidden' name='cuCountyOfficeBulletinBoards[" + index + "].country.id' class='countryId'/>"
                + "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--办公场地结构--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyOfficeBulletinBoards[" + index + "].officeStructure' class='input_none'/>"
                + "</td>"
                <%--办公场地面积--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyOfficeBulletinBoards[" + index + "].officeArea' class='input_none decimal' maxlength='8'/>"
                + "</td>"
                <%--宣传栏数量--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyOfficeBulletinBoards[" + index + "].bulletinBoardNum' class='input_none int' maxlength='8'/>"
                + "</td>"
                <%--宣传栏面积--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyOfficeBulletinBoards[" + index + "].bulletinBoardArea' class='input_none decimal' maxlength='8'/>"
                + "</td>"
                <%--备注--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyOfficeBulletinBoards[" + index + "].remarks' class='input_none'/>"
                + "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #OfficeBulletinBoard").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyOfficeBulletinBoard/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县（市、区）行政村有办公场所、宣传栏情况表</h1>
			<br/><br/>
			<p>
				...
				<span class="county">填报单位：
					...
					<input name="department.name" class="underline required countyName"
						   value="${cuTable.department.name}"/>县(市、区）委组织部（盖章）
				</span>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="60px" colspan="2">办公场所</td>
						<td width="55px" colspan="2">宣传栏</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyOfficeBulletinBoards}">
						<c:forEach items="${cuTable.cuCountyOfficeBulletinBoards}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyOfficeBulletinBoards[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								...
								<td class='bg-white town'>
									<input type='hidden'
										   name='cuCountyOfficeBulletinBoards[${index}].town.id'
										   class='townId'
										   value='${house.town.id}'/>
									...
									<%--村--%>
								<td class='bg-white country'>
									<input type='hidden' name='cuCountyOfficeBulletinBoards[${index}].country.id'
										   class='countryId '
									...
									<%--办公场地结构--%>
								<td class='bg-white'>
									<input name='cuCountyOfficeBulletinBoards[${index}].officeStructure'
										   class='input_none'
										   value='${house.officeStructure}'/>
								</td>
									<%--办公场地面积--%>
								<td class='bg-white'>
									<input name='cuCountyOfficeBulletinBoards[${index}].officeArea'
										   class='input_none decimal'
										   maxlength='8'
										   value='${house.officeArea}'/>
								</td>
									<%--宣传栏数量--%>
								<td class='bg-white'>
									<input name='cuCountyOfficeBulletinBoards[${index}].bulletinBoardNum'
										   class='input_none int'
										   maxlength='8'
										   value='${house.bulletinBoardNum}'/>
								</td>
									<%--宣传栏面积--%>
								<td class='bg-white'>
									<input name='cuCountyOfficeBulletinBoards[${index}].bulletinBoardArea'
										   class='input_none decimal'
										   maxlength='8'
										   value='${house.bulletinBoardArea}'/>
								</td>
									<%--备注--%>
								<td class='bg-white'>
									<input name='cuCountyOfficeBulletinBoards[${index}].remarks'
										   class='input_none'
										   value='${house.remarks}'/>
								</td>
								<td class='bg-white' style='text-align: center'>
