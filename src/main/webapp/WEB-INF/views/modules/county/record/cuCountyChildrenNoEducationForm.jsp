	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
                ...
                + "<input type='hidden' name='cuCountyChildrenNoEducations[" + index + "].town.id' class='townId'/>"
                + "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--村--%>
                + "<td class='bg-white country'>"
                + "<input type='hidden' name='cuCountyChildrenNoEducations[" + index + "].country.id' class='countryId'/>"
                + "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"

                <%--自然村--%>
                + "<td class='bg-white tun'>"
                + "<input type='hidden' name='cuCountyChildrenNoEducations[" + index + "].tun.id' class='tunId'/>"
                + "<input class='input_none required tunName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--适龄儿童少年应接受义务教育但未接受人员姓名--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyChildrenNoEducations[" + index + "].noEducationChildren' class='input_none realName required'/>"
                + "</td>"
                <%--户主姓名--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyChildrenNoEducations[" + index + "].householdName' class='input_none realName'/>"
                + "</td>"
                <%--辍学原因--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyChildrenNoEducations[" + index + "].dropOutReason' class='input_none'/>"
                + "</td>"
                <%--入户劝返次数--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyChildrenNoEducations[" + index + "].returnNum' class='input_none int' maxlength='3'/>"
                + "</td>"
                <%--备注--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyChildrenNoEducations[" + index + "].remarks' class='input_none'/>"
                + "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #ChildrenNoEducation").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyChildrenNoEducation/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县(市、区)适龄儿童少年应接受义务教育未接受名单</h1>
			<br/><br/>
			<p>
				...
				<span class="county">填报单位：
					...
					<input name="department.name" class="underline required countyName"
						   value="${cuTable.department.name}"/>县(市、区）教育局（盖章）
				</span>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="60px">自然村（屯）</td>
						<td width="55px">未接受儿童少年姓名</td>
						<td width="55px">户主姓名</td>
						<td width="260px">辍学原因</td>
						<td width="20px">入户劝返次数</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyChildrenNoEducations}">
						<c:forEach items="${cuTable.cuCountyChildrenNoEducations}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyChildrenNoEducations[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								<td class="bg-white">${index}</td>
									<%--乡镇--%>
								<td class="bg-white town">
									<input type="hidden"
										   name="cuCountyChildrenNoEducations[${index}].town.id"
										   class="townId"
										   value="${house.town.id}"/>
									...
									<%--村--%>
								<td class="bg-white country">
									<input type="hidden" name="cuCountyChildrenNoEducations[${index}].country.id"
										   class="countryId "
									...
									<%--自然村--%>
								<td class="bg-white country">
									<input type="hidden" name="cuCountyChildrenNoEducations[${index}].tun.id"
										   class="tunId "
										   value="${house.tun.id}"/>
									...
									<%--适龄儿童少年应接受义务教育但未接受人员姓名--%>
								<td class="bg-white">
									<input name="cuCountyChildrenNoEducations[${index}].noEducationChildren"
										   class="input_none realName required"
										   value="${house.noEducationChildren}"/>
								</td>
									<%--户主姓名--%>
								<td class="bg-white">
									<input name="cuCountyChildrenNoEducations[${index}].householdName"
										   class="input_none realName"
										   value="${house.householdName}"/>
								</td>
									<%--辍学原因--%>
								<td class="bg-white">
									<input name="cuCountyChildrenNoEducations[${index}].dropOutReason"
										   class="input_none"
										   value="${house.dropOutReason}"/>
								</td>
									<%--入户劝返次数--%>
								<td class="bg-white">
									<input name="cuCountyChildrenNoEducations[${index}].returnNum"
										   class="input_none int"
										   maxlength="3"
										   value="${house.returnNum}"/>
								</td>
									<%--备注--%>
								<td class="bg-white">
									<input name="cuCountyChildrenNoEducations[${index}].remarks"
										   class="input_none"
										   value="${house.remarks}"/>
								</td>
								<td class="bg-white" style="text-align: center">
			...
			<div class="form-actions" style="padding-left: 20px!important;">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountyChildrenNoEducation:edit">
