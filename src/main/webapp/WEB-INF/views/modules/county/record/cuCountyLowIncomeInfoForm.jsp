...
<html>
<head>
    <title>县贫困户纳入农村低保户情况统计表管理</title>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(function () {
                $(".content-nav #LowIncome").attr("class", "active");
            });
        });
    </script>
</head>
...
<div class="content tabs f-clear">
    ...
    <div id="tab-1" class="content-detail">
        ...
        <form:form id="inputForm" modelAttribute="cuTable"
                   action="${ctx}/record/cuCountyLowIncomeInfo/save" method="post"
            ...
            <div>
                <table>
                    ...
                    <tr>
                        ...
                        <td width="100px">贫困户数</td>
                        <td width="100px">符合条件应纳入农村低保户数</td>
                        <td width="100px">已纳入农村低保户数</td>
                        <td width="80px">符合条件纳入率</td>
                        <td width="80px">备注</td>
                    ...
                    <tbody id="accpitem">
                    <c:if test="${!empty cuTable.cuCountyLowIncomeInfos}">
                        <c:forEach items="${cuTable.cuCountyLowIncomeInfos}" var="county" varStatus="status">
                            <c:set var="indexN" value="${status.index+1}" scope="page"/>
                            <input type="hidden" class="id" name="cuCountyLowIncomeInfos[${status.index}].id"
                                   value="${county.id}"/>
                            <tr>
                                <td class="bg-white">${indexN}</td>
                                <td class="bg-white required town">
                                    <input type="hidden" class="townId"
                                           name="cuCountyLowIncomeInfos[${status.index}].town.id"
                                           value="${county.town.id}"/>
                                    <input class="td-input townName required"
                                           name="cuCountyLowIncomeInfos[${status.index}].town.name"
                                           readonly="true" value="${county.town.name}"/>
                                </td>
                                <td class="bg-white required village">
                                <input type="hidden" class="villageId"
                                       name="cuCountyLowIncomeInfos[${status.index}].village.id"
                                       value="${county.village.id}"/>
                                <input class="td-input villageName required"
                                       name="cuCountyLowIncomeInfos[${status.index}].village.name"
                                       readonly="true" value="${county.village.name}"/>
                            </td>
                                <td class="bg-white">
                                    <input class="houseNumber int td-input required"
                                           name="cuCountyLowIncomeInfos[${status.index}].houseNumber"
                                           value="${county.houseNumber}" maxlength="8"/>
                                </td>
                                <td class="bg-white">
                                    <input class="matchConditionNumber int td-input required"
                                           name="cuCountyLowIncomeInfos[${status.index}].matchConditionNumber"
                                           value="${county.matchConditionNumber}" maxlength="8"/>
                                </td>
                                <td class="bg-white">
                                    <input class="lowIncomeNumber td-input int required"
                                           name="cuCountyLowIncomeInfos[${status.index}].lowIncomeNumber"
                                           value="${county.lowIncomeNumber}" maxlength="8"/>
                                </td>
                                <td>
                                    ${county.rate}
                                </td>
                                <td class="bg-white">
                                    <input class="remarks td-input"
                                           name="cuCountyLowIncomeInfos[${status.index}].remarks"
            ...
            <div class="form-actions actions">
                <div class="submitStyle">
                    <shiro:hasPermission name="record:cuCountyLowIncomeInfo:edit">
    ...
    $(function () {
        $(".add-tr-btn").click(function () {
            ...
            var tr = '<tr>' +
                ...
                '<input type="hidden" class="townId" name="cuCountyLowIncomeInfos[' + num + '].town.id"/>' +
                '<input class="td-input townName required" readonly="true" name="cuCountyLowIncomeInfos[' + num + '].town.name"/>' +
                '</td>' +
                ' <td class="bg-white village">' +
                '<input type="hidden" class="villageId" name="cuCountyLowIncomeInfos[' + num + '].village.id"/>' +
                '<input class="td-input villageName required" readonly="true" name="cuCountyLowIncomeInfos[' + num + '].village.name"/>' +
                '</td>' +
                '<td class="bg-white">' +
                '<input class="td-input houseNumber int required" maxlength="8" name="cuCountyLowIncomeInfos[' + num + '].houseNumber"/> ' +
                '</td>' +
                '<td class="bg-white">' +
                '<input class="td-input matchConditionNumber int required" maxlength="8" name="cuCountyLowIncomeInfos[' + num + '].matchConditionNumber"/> ' +
                '</td>' +
                '<td class="bg-white">' +
                '<input class="td-input lowIncomeNumber int required" maxlength="8" name="cuCountyLowIncomeInfos[' + num + '].lowIncomeNumber"/> ' +
                '</td>' +
                '<td>' +
                '</td>' +
                '<td class="bg-white">' +
                '<input class="td-input remarks" name="cuCountyLowIncomeInfos[' + num + '].remarks" maxlength="500"/>' +
                '</td>' +
                '<td class="bg-white" style="text-align: center">' +
                '<span class="del-thistd-btn">删除</span>' +
                '</td>' +
