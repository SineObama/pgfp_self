...
<html>
<head>
    <title>县农村居民未购买医疗保险或商业保险名单</title>
        ...
        $(document).ready(function () {
            ...
            $('.btnEx').click(function () {
                top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v) {
                    var cuTable = null;
                    if (v == "ok") {
                        window.location.href = "${ctx}/record/cuCountyNoInsurance/export?type=${cuTable.type}&cuMainId=${cuTable.cuMainId}";
                    }
                });
            });

            $(function () {
                $(".content-nav #noInsurance").attr("class", "active");
            });
...
<div class="content tabs f-clear">
    ...
    <div id="tab-1" class="content-detail">
        ...
        <form:form id="inputForm" modelAttribute="cuTable"
                   action="${ctx}/record/cuCountyNoInsurance/save" method="post"
            ...
            <div>
                <table>
                    ...
                    <tr>
                        ...
                        <td>户主姓名</td>
                        <td>家庭人口数</td>
                        <td style="width:200px!important;">未购买原因</td>
                        <td>备注</td>
                    ...
                    <tbody id="accpitem">
                    <c:if test="${!empty cuTable.cuCountyNoInsurances}">
                        <c:forEach items="${cuTable.cuCountyNoInsurances}" var="county" varStatus="status">
                            <c:set var="indexN" value="${status.index+1}" scope="page"/>
                            <input type="hidden" class="id" name="cuCountyNoInsurances[${status.index}].id"
                                   value="${county.id}"/>
                            <tr>
                                <td class="bg-white">${indexN}</td>
                                <td class="bg-white required town">
                                    <input type="hidden" class="townId"
                                           name="cuCountyNoInsurances[${status.index}].town.id"
                                           value="${county.town.id}"/>
                                    <input class="td-input townName required"
                                           name="cuCountyNoInsurances[${status.index}].town.name"
                                           readonly="true" value="${county.town.name}"/>
                                </td>
                                <td class="bg-white village">
                                    <input type="hidden" class="villageId"
                                           name="cuCountyNoInsurances[${status.index}].village.id"
                                           value="${county.village.id}"/>
                                    <input class="td-input villageName required"
                                           name="cuCountyNoInsurances[${status.index}].village.name"
                                           readonly="true" value="${county.village.name}"/>
                                </td>
                                <td class="bg-white tun">
                                    <input type="hidden" class="tunId"
                                           name="cuCountyNoInsurances[${status.index}].tun.id"
                                           value="${county.tun.id}"/>
                                    <input class="td-input tunName required"
                                           name="cuCountyNoInsurances[${status.index}].tun.name"
                                           readonly="true" value="${county.tun.name}"/>
                                </td>
                                <td class="bg-white">
                                    <input class="householdName realName td-input required"
                                           name="cuCountyNoInsurances[${status.index}].householdName"
                                           value="${county.householdName}" maxlength="30"/>
                                </td>
                                <td class="bg-white">
                                    <input class="familyMember int td-input required"
                                           name="cuCountyNoInsurances[${status.index}].familyMember"
                                           value="${county.familyMember}" maxlength="8"/>
                                </td>
                                <td class="bg-white">
                                    <input class="nobuyReason td-input required"
                                           name="cuCountyNoInsurances[${status.index}].nobuyReason"
                                           value="${county.nobuyReason}" maxlength="200"/>
                                </td>
                                <td class="bg-white">
                                    <input class="remarks td-input"
                                           name="cuCountyNoInsurances[${status.index}].remarks"
            ...
            <div class="form-actions actions">
                <div class="submitStyle">
                    <shiro:hasPermission name="record:cuCountyNoInsurance:edit">
...
<script>
    ...
    $(function () {
        $(".add-tr-btn").click(function () {
            ...
            var num = index + renum;
            var tr = '<tr>' +
                '<input type="hidden" class="id"  name="cuCountyNoInsurances[' + num + '].id"/>' +
                '<td class="bg-white">' +
                '</td>' +
                '<td class="bg-white town">' +
                '<input type="hidden" class="townId" name="cuCountyNoInsurances[' + num + '].town.id"/>' +
                '<input class="td-input townName required" readonly="true" name="cuCountyNoInsurances[' + num + '].town.name"/>' +
                '</td>' +
                ' <td class="bg-white village">' +
                '<input type="hidden" class="villageId" name="cuCountyNoInsurances[' + num + '].village.id"/>' +
                '<input class="td-input villageName required" readonly="true" name="cuCountyNoInsurances[' + num + '].village.name"/>' +
                '</td>' +
                ' <td class="bg-white tun">' +
                '<input type="hidden" class="tunId" name="cuCountyNoInsurances[' + num + '].tun.id"/>' +
                '<input class="td-input tunName required" readonly="true" name="cuCountyNoInsurances[' + num + '].tun.name"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input householdName realName required" maxlength="30" name="cuCountyNoInsurances[' + num + '].householdName"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input familyMember int required" maxlength="8" name="cuCountyNoInsurances[' + num + '].familyMember"/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input nobuyReason required" maxlength="200" name="cuCountyNoInsurances[' + num + '].nobuyReason"/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input remarks" name="cuCountyNoInsurances[' + num + '].remarks" maxlength="500"/>' +
                '</td>' +
                '<td class="bg-white" style="text-align: center">' +
                '<span class="del-thistd-btn">删除</span>' +
                '</td>' +
