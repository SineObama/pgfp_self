...
<html>
<head>
    <title>县城乡居民养老保险未参保人员名单管理</title>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(function () {
                $(".content-nav #NoEndowmentInsurance").attr("class", "active");
            });
...
<div class="content tabs f-clear">
    ...
    <div id="tab-1" class="content-detail">
        ...
        <form:form id="inputForm" modelAttribute="cuTable"
                   action="${ctx}/record/cuCountyNoEndowmentInsurance/save" method="post"
            ...
            <div>
                <table>
                    ...
                    <tr>
                        ...
                        <td width="120px">未参加城乡居民养老保险人员姓名</td>
                        <td width="120px">未参加城乡居民养老保险人员户主姓名</td>
                        <td width="180px">未参保主要原因</td>
                        <td width="80px">备注</td>
                    ...
                    <tbody id="accpitem">
                    <c:if test="${!empty cuTable.cuCountyNoEndowmentInsurances}">
                        <c:forEach items="${cuTable.cuCountyNoEndowmentInsurances}" var="county" varStatus="status">
                            <c:set var="indexN" value="${status.index+1}" scope="page"/>
                            <input type="hidden" class="id" name="cuCountyNoEndowmentInsurances[${status.index}].id"
                                   value="${county.id}"/>
                            <tr>
                                <td class="bg-white">${indexN}</td>
                                <td class="bg-white required town">
                                    <input type="hidden" class="townId"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].town.id"
                                           value="${county.town.id}"/>
                                    <input class="td-input townName required"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].town.name"
                                           readonly="true" value="${county.town.name}"/>
                                </td>
                                <td class="bg-white village">
                                    <input type="hidden" class="villageId"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].village.id"
                                           value="${county.village.id}"/>
                                    <input class="td-input villageName required"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].village.name"
                                           readonly="true" value="${county.village.name}"/>
                                </td>
                                <td class="bg-white tun">
                                    <input type="hidden" class="tunId"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].tun.id"
                                           value="${county.tun.id}"/>
                                    <input class="td-input tunName required"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].tun.name"
                                           readonly="true" value="${county.tun.name}"/>
                                </td>
                                <td class="bg-white">
                                    <input class="name realName td-input required"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].name"
                                           value="${county.name}" maxlength="30"/>
                                </td>
                                <td class="bg-white">
                                    <input class="householdName realName td-input required"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].householdName"
                                           value="${county.householdName}" maxlength="30"/>
                                </td>
                                <td class="bg-white">
                                    <input class="reason td-input required" style="width:120px!important; ;"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].reason"
                                           value="${county.reason}" maxlength="200"/>
                                </td>
                                <td class="bg-white">
                                    <input class="remarks td-input"
                                           name="cuCountyNoEndowmentInsurances[${status.index}].remarks"
            ...
            <div class="form-actions actions">
                <div class="submitStyle">
                    <shiro:hasPermission name="record:cuCountyNoEndowmentInsurance:edit">
...
<script>
    ...
    $(function () {
        $(".add-tr-btn").click(function () {
            ...
            var tr = '<tr>' +
                '<input type="hidden" class="id"  name="cuCountyNoEndowmentInsurances[' + index + '].id"/>' +
                '<td class="bg-white">' +
                '</td>' +
                '<td class="bg-white town">' +
                '<input type="hidden" class="townId" name="cuCountyNoEndowmentInsurances[' + index + '].town.id"/>' +
                '<input class="td-input townName required" readonly="true" name="cuCountyNoEndowmentInsurances[' + index + '].town.name"/>' +
                '</td>' +
                ' <td class="bg-white village">' +
                '<input type="hidden" class="villageId" name="cuCountyNoEndowmentInsurances[' + index + '].village.id"/>' +
                '<input class="td-input villageName required" readonly="true" name="cuCountyNoEndowmentInsurances[' + index + '].village.name"/>' +
                '</td>' +
                ' <td class="bg-white tun">' +
                '<input type="hidden" class="tunId" name="cuCountyNoEndowmentInsurances[' + index + '].tun.id"/>' +
                '<input class="td-input tunName required" readonly="true" name="cuCountyNoEndowmentInsurances[' + index + '].tun.name"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input name realName required" maxlength="30" name="cuCountyNoEndowmentInsurances[' + index + '].name"/>' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input householdName realName required" maxlength="30" name="cuCountyNoEndowmentInsurances[' + index + '].householdName"/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input reason required" maxlength="200" name="cuCountyNoEndowmentInsurances[' + index + '].reason" style="width:120px!important;"/> ' +
                '</td>' +
                ' <td class="bg-white">' +
                '<input class="td-input remarks" name="cuCountyNoEndowmentInsurances[' + index + '].remarks" maxlength="500"/>' +
                '</td>' +
                '<td class="bg-white" style="text-align: center">' +
                '<span class="del-thistd-btn">删除</span>' +
                '</td>' +
