	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
                ...
                + "<input type='hidden' name='cuCountyNetworkings[" + index + "].town.id' class='townId'/>"
                + "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--村--%>
                + "<td class='bg-white country'>"
                + "<input type='hidden' name='cuCountyNetworkings[" + index + "].country.id' class='countryId'/>"
                + "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--通网络宽带方式--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyNetworkings[" + index + "].way' class='input_none'/>"
                + "</td>"
                <%--通达地点--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyNetworkings[" + index + "].adress' class='input_none'/>"
                + "</td>"
                <%--备注--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyNetworkings[" + index + "].remarks' class='input_none'/>"
                + "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #Networking").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyNetworking/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县(市、区）行政村通网络宽带情况统计表</h1>
			<br/><br/>
			<p>
				...
				<span class="county">填报单位：
					...
					<input name="department.name" class="underline required countyName"
						   value="${cuTable.department.name}"/>县(市、区）基础设施专责小组（盖章）
				</span>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="80px">通网络宽带方式</td>
						<td width="160px">通达地点</td>
						<td width="180px" rowspan="2">备注</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyNetworkings}">
						<c:forEach items="${cuTable.cuCountyNetworkings}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyNetworkings[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								...
								<td class='bg-white town'>
									<input type='hidden'
										   name='cuCountyNetworkings[${index}].town.id'
										   class='townId'
										   value='${house.town.id}'/>
									...
									<%--村--%>
								<td class='bg-white country'>
									<input type='hidden' name='cuCountyNetworkings[${index}].country.id'
										   class='countryId '
									...
									<%--通网络宽带方式--%>
								<td class='bg-white'>
									<input name='cuCountyNetworkings[${index}].way'
										   class='input_none'
										   value='${house.way}'/>
								</td>
									<%--通达地点--%>
								<td class='bg-white'>
									<input name='cuCountyNetworkings[${index}].adress'
										   class='input_none'
										   value='${house.adress}'/>
								</td>
									<%--备注--%>
								<td class='bg-white'>
									<input name='cuCountyNetworkings[${index}].remarks'
										   class='input_none'
										   value='${house.remarks}'/>
								</td>
								<td class='bg-white' style='text-align: center'>
			...
			<div class="form-actions" style="padding-left: 20px!important;">
				<div class="submitStyle">
					<shiro:hasPermission name="record:cuCountyNetworking:edit">
