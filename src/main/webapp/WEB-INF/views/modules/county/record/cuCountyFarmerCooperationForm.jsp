...
<head>
	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr ="<tr>"
                    ...
                    + "<td class='bg-white town'>"
                        + "<input type='hidden'name='cuCountyFarmerCooperations[" + index + "].town.id'class='townId'/>"
                        + "<input class='input_none townName required'style='background-color: #FFFFFF'readonly='true'/>"
                    + "</td>"
                    <%--村--%>
                    + "<td class='bg-white country'>"
                        + "<input type='hidden' name='cuCountyFarmerCooperations[" + index + "].country.id'class='countryId'/>"
                        + "<input class='input_none required countryName'style='background-color: #FFFFFF'readonly='true'/>"
                    + "</td>"
                    <%--合作社名称--%>
                        + "<td class='bg-white'>"
                        + "<input name='cuCountyFarmerCooperations[" + index + "].cooperationName'class='input_none required' style='width: 160px!important;'/>"
                    + "</td>"
                    <%--合作社法人--%>
                    + "<td class='bg-white'>"
                        + "<input name='cuCountyFarmerCooperations[" + index + "].cooperativeCorporation'class='input_none required'/>"
                    + "</td>"
                    <%--覆盖贫困户数--%>
                    + "<td class='bg-white'>"
                        + "<input name='cuCountyFarmerCooperations[" + index + "].coveragePoorHouse'class='input_none required int'/>"
                    + "</td>"
                    <%--备注--%>
                    + "<td class='bg-white'>"
                        + "<input name='cuCountyFarmerCooperations[" + index + "].remarks'class='input_none'/>"
                    + "</td>"
        ...
        $(document).ready(function () {
			...
			/*菜单置为选中状态*/
            $(function () {
                $(".content-nav #artel").attr("class", "active");
            });
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyFarmerCooperation/save" method="post" class="form-horizontal">
			<sys:message content="${message}"/>
			<form:hidden id="year" path="year"/>
			<form:hidden id="cuMainId" path="cuMainId"/>
			<h1 style="text-align: center"><u> 平桂 </u>县(市、区）贫困村农民合作社一览表</h1>
			<br/><br/>
			<p>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="80px">贫困村</td>
						<td width="140px">合作社名称</td>
						<td width="60px">合作社法人</td>
						<td width="60px">覆盖贫困户数</td>
						<td width="60px">备注</td>
						<td tyle="cursor: pointer;" class="add-tr-btn" width="40px">添加<span
								style="color: #fe0000;font-size: 16px;font-weight: bold;">+</span></td>
					</tr>
					</thead>
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyFarmerCooperations}">
						<c:forEach items="${cuTable.cuCountyFarmerCooperations}" var="county" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyFarmerCooperations[${index}].id" value="${county.id}"/>
							<tr>
                                    <%--序号--%>
								...
								<td class="bg-white town">
                                    <input type="hidden"
                                           name="cuCountyFarmerCooperations[${index}].town.id"
                                           class="townId"
                                           value="${county.town.id}"/>
									<input class="input_none townName required"
                                           style="background-color: #FFFFFF"
                                           value="${county.town.name}"
                                           readonly="true"/>
								</td>
                                    <%--村--%>
                                <td class="bg-white country">
                                    <input type="hidden" name="cuCountyFarmerCooperations[${index}].country.id"
                                           class="countryId "
                                           value="${county.country.id}"/>
                                    <input class="input_none required countryName required"
                                           style="background-color: #FFFFFF"
                                           readonly="true"
                                           value="${county.country.name}"/>
                                </td>
                                    <%--合作社名称--%>
                                <td class="bg-white">
                                    <input name="cuCountyFarmerCooperations[${index}].cooperationName"
										   style="width: 160px!important;"
										   class="input_none required"
										   value="${county.cooperationName}"/>
                                </td>
                                    <%--合作社法人--%>
								<td class="bg-white">
                                    <input name="cuCountyFarmerCooperations[${index}].cooperativeCorporation"
										   class="input_none required realName"
										   value="${county.cooperativeCorporation}"/>
                                </td>
                                     <%--覆盖贫困户数--%>
                                <td class="bg-white">
                                    <input name="cuCountyFarmerCooperations[${index}].coveragePoorHouse"
										   class="input_none required int"
										   value="${county.coveragePoorHouse}"/>
                                </td>
                                     <%--备注--%>
                                <td class="bg-white">
                                     <input name="cuCountyFarmerCooperations[${index}].remarks"
											class="input_none"
