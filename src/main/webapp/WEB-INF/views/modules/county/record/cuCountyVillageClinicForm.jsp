	...
	<script type="text/javascript">
        ...
        function addTr() {
            ...
            var tr = "<tr>"
                ...
                + "<input type='hidden' name='cuCountyVillageClinics[" + index + "].town.id' class='townId'/>"
                + "<input class='input_none townName required' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--行政村--%>
                + "<td class='bg-white country'>"
                + "<input type='hidden' name='cuCountyVillageClinics[" + index + "].country.id' class='countryId'/>"
                + "<input class='input_none required countryName' style='background-color: #FFFFFF' readonly='true'/>"
                + "</td>"
                <%--行政村是否为乡(镇)人民政府地--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyVillageClinics[" + index + "].government' class='input_none required' maxlength='1'/>"
                + "</td>"
                <%--卫生室名称--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyVillageClinics[" + index + "].clinic' class='input_none'/>"
                + "</td>"
                <%--是否为标准化卫生室--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyVillageClinics[" + index + "].standard' class='input_none' maxlength='1'/>"
                + "</td>"
                <%--负责人姓名--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyVillageClinics[" + index + "].principal' class='input_none realName'/>"
                + "</td>"
                <%--联系方式--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyVillageClinics[" + index + "].phone' class='input_none mobile'/>"
                + "</td>"
                <%--备注--%>
                + "<td class='bg-white'>"
                + "<input name='cuCountyVillageClinics[" + index + "].remarks' class='input_none'/>"
                + "</td>"
        ...
        $(document).ready(function () {
            ...
            /*菜单置为选中状态*/
            $(".content-nav #VillageClinic").attr("class", "active");
        })
...
<div class="content tabs f-clear">
	...
	<div id="tab-1" class="content-detail">
		<form:form id="inputForm" modelAttribute="cuTable"
				   action="${ctx}/record/cuCountyVillageClinic/save" method="post" class="form-horizontal">
			...
			<p>
				...
				<span class="county">填报单位：
					...
					<input name="department.name" class="underline required countyName"
						   value="${cuTable.department.name}"/>县(市、区）卫生计生局（盖章）
				</span>
			...
			<div>
				<table>
					...
					<tr>
						...
						<td width="60px">行政村是否为乡(镇)人民政府地</td>
						<td width="100px">卫生室名称</td>
						<td width="40px">是否为标准化卫生室</td>
						<td width="70px">负责人姓名</td>
					...
					<tbody id="accpitem">
					<c:if test="${!empty cuTable.cuCountyVillageClinics}">
						<c:forEach items="${cuTable.cuCountyVillageClinics}" var="house" varStatus="status">
							<c:set var="index" value="${status.index}"/>
							<input type="hidden" name="cuCountyVillageClinics[${index}].id" value="${house.id}"/>
							<tr>
									<%--序号--%>
								<td class='bg-white'>${index}</td>
									<%--乡镇--%>
								<td class='bg-white town'>
									<input type='hidden'
										   name='cuCountyVillageClinics[${index}].town.id'
										   class='townId'
										   value='${house.town.id}'/>
									...
									<%--行政村--%>
								<td class='bg-white country'>
									<input type='hidden' name='cuCountyVillageClinics[${index}].country.id'
										   class='countryId '
									...
									<%--行政村是否为乡(镇)人民政府地--%>
								<td class='bg-white'>
									<input name='cuCountyVillageClinics[${index}].government'
										   class='input_none required'
										   maxlength="1"
										   value='${house.government}'/>
								</td>
									<%--卫生室名称--%>
								<td class='bg-white'>
									<input name='cuCountyVillageClinics[${index}].clinic'
										   class='input_none'
										   value='${house.clinic}'/>
								</td>
									<%--是否为标准化卫生室--%>
								<td class='bg-white'>
									<input name='cuCountyVillageClinics[${index}].standard'
										   class='input_none'
										   maxlength="1"
										   value='${house.standard}'/>
								</td>
									<%--负责人姓名--%>
								<td class='bg-white'>
									<input name='cuCountyVillageClinics[${index}].principal'
										   class='input_none realName'
										   value='${house.principal}'/>
								</td>
									<%--联系方式--%>
								<td class='bg-white'>
									<input name='cuCountyVillageClinics[${index}].phone'
										   class='input_none mobile'
										   value='${house.phone}'/>
								</td>
									<%--备注--%>
								<td class='bg-white'>
									<input name='cuCountyVillageClinics[${index}].remarks'
										   class='input_none'
										   value='${house.remarks}'/>
								</td>
								<td class='bg-white' style='text-align: center'>
