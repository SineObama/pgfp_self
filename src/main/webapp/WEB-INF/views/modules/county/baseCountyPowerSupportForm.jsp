...
<body>
	<ul class="nav nav-tabs">
		...
		<%-- <li><a href="${ctx}/county/baseCountyPowerSupport/">农村电力保障列表</a></li> --%>
		<li class="active"><a href="${ctx}/county/baseCountyPowerSupport/form?baseCountyInfoId=${baseCountyKeyWorkInfo.baseCountyInfoId}&type=3">农村电力保障<shiro:hasPermission name="county:baseCountyPowerSupport:edit">${not empty baseCountyKeyWorkInfo.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="county:baseCountyPowerSupport:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	...
	<c:choose>
		...
		<c:otherwise>
			<form:form id="inputForm" modelAttribute="baseCountyKeyWorkInfo" action="${ctx}/county/baseCountyPowerSupport/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table>
                <tbody>
                <tr>
                    <td class="fir-td"><span>通电行政村数（个）</span></td>
                    <td class="sec-td bg-white">
                    	<form:input path="electrifyVillages" htmlEscape="false" maxlength="8" class="input-xlarge  digits"/>
                    </td>
                    <td class="thr-td"><span>能源部门资金投入（万元）</span></td>
                    <td class="fou-td bg-white">
                    	<form:input path="energyDepFunding" maxlength="11" htmlEscape="false" class="input-xlarge  decimal"/>
                    </td>
                </tr>
                <tr>
                    <td class="fir-td"><span>其中：通生产用电行政村数（个）</span></td>
                    <td class="sec-td bg-white">
                        <form:input path="productionElectricityVillages" htmlEscape="false" maxlength="8" class="input-xlarge  digits"/>
                    </td>
                    <td class="thr-td"><span>其中：中央资金投入（万元）</span></td>
                    <td class="fou-td bg-white">
						<form:input path="centralFundingForElectrify" maxlength="11" htmlEscape="false" class="input-xlarge  decimal"/>
                    </td>
                </tr>
                <tr>
                    <td class="fir-td"><span>通电户数（万户）</span></td>
                    <td class="sec-td bg-white">
                    	<form:input path="electrifyHouseholds" maxlength="11"  htmlEscape="false" class="input-xlarge  decimal"/>
                    </td>
                    <td class="thr-td"><span>省级资金投入（万元）</span></td>
                    <td class="fou-td bg-white">
                    	<form:input path="provincialFundingForElectrify" maxlength="11" htmlEscape="false" class="input-xlarge  decimal"/>
                    </td>
                </tr>
                <tr>
                    <td class="fir-td"><span>无电人口数（万人）</span></td>
                    <td class="sec-td bg-white">
                    	<form:input path="noElectrifyPopulation" maxlength="11" htmlEscape="false" class="input-xlarge  decimal"/>
                    </td>
                    <td class="thr-td"><span>市县资金投入（万元）</span></td>
                    <td class="fou-td bg-white">
                    	<form:input path="countyFundingForElectrify" maxlength="11" htmlEscape="false" class="input-xlarge  decimal"/>
                    </td>
                </tr>
