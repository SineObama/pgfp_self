...
<ul class="nav nav-tabs">
	...
	<li class="active"><a href="">住房未达标名单情况表<shiro:hasPermission
...
<div class="content tabs f-clear">
	<ul class="content-nav">
		...
		<li class="active"><a>住房保障未达标户名单情况表</a></li>
	...
	<div id="tab-15" class="content-detail">
		<c:choose>
			...
			<c:otherwise>
				<form:form id="inputForm" modelAttribute="comain" action="${ctx}/co/coHouseSituation/save" method="post"
						   class="form-horizontal">
					<sys:message content="${message}"/>
					<form:hidden path="id" value="${comain.id}"/>
					<form:hidden path="coId"/>
					<h2 class="table-title"><input type="text" value="${countryName}" readonly/>住房保障未达标户名单情况表
					</h2>

					<h3 class="table-subtitle">
						<span>填报单位：<input type="text" value="平桂" readonly/>县（市丶区）住房城乡建设部门（盖章）</span>
					...
					<table>
						...
						<tr>
							...
							<td>住房保障未达标户户主姓名</td>
							<td>家庭人口数</td>
							<td style="min-width: 200px!important;">未达标原因</td>
						...
						<c:if test="${comain.coHouseSituationList != null && !comain.coHouseSituationList.isEmpty()}">
						<c:forEach items="${comain.coHouseSituationList}" var="coHouseSituation" varStatus="status">
						<tr>
							<td class="bg-white">
								<input type="hidden" name="coHouseSituationList[${status.index}].id"
									   value="${coHouseSituation.id}" class="id"/>
								<input type="hidden" name="coHouseSituationList[${status.index}].mainId" class="mainId"
									   value="${coHouseSituation.mainId}"/>
								<form:input path="coHouseSituationList[${status.index}].idx" htmlEscape="false"
											maxlength="255" class="idx input-xlarge "/>
							</td>
							<td class="bg-white">
								<form:input path="coHouseSituationList[${status.index}].natureId" htmlEscape="false"
											maxlength="64" class="natureId input-xlarge"/>
							</td>
							<td class="bg-white">
								<form:input path="coHouseSituationList[${status.index}].housename" htmlEscape="false"
											maxlength="64" class="housename input-xlarge"/>
							</td>
							<td class="bg-white">
								<form:input path="coHouseSituationList[${status.index}].familyNumber" htmlEscape="false"
											maxlength="64" class="familyNumber input-xlarge int"/>
							</td>

							<td class="bg-white">
								<form:input path="coHouseSituationList[${status.index}].reason" htmlEscape="false"
											maxlength="64" class="reason input-xlarge"/>
							</td>
							<td class="bg-white">
								<form:input path="coHouseSituationList[${status.index}].solve" htmlEscape="false"
											maxlength="64" class="solve input-xlarge"/>
							</td>
							<td class="bg-white">
								<form:input path="coHouseSituationList[${status.index}].remarks" htmlEscape="false"
...
//				var cardId = $("#id").val();
		...
		$(function () {
			...
			$(".add-tr-btn").click(function () {
				...
				var trHtml = '<tr>' +
						'<td class="bg-white">' +
						'<input type="hidden" name="coHouseSituationList[' + _len + '].id"  class="id">' +
						'<input type="hidden" name="coHouseSituationList[' + _len + '].mainId" class="mainId">' +
						'<input id="coHouseSituationList' + _len + '.idx" name="coHouseSituationList[' + _len + '].idx" class="idx input-mini required " type="text"  maxlength="10">' +
						'</td>' +
						'<td class="bg-white">' +
						'<input id="coHouseSituationList' + _len + '.natureId" name="coHouseSituationList[' + _len + '].natureId" class="natureId" type="text" >' +
						'</td>' +
						'<td class="bg-white">' +
						'<input id="coHouseSituationList' + _len + '.housename" name="coHouseSituationList[' + _len + '].housename" class="housename" type="text" >' +
						'</td>' +
						'<td class="bg-white">' +
						'<input id="coHouseSituationList' + _len + '.familyNumber" name="coHouseSituationList[' + _len + '].familyNumber" class="familyNumber int" type="text" >' +
						'</td>' +
						'<td class="bg-white">' +
						'<input id="coHouseSituationList' + _len + '.reason" name="coHouseSituationList[' + _len + '].reason" class="reason " type="text" >' +
						'</td>' +
						'<td class="bg-white">' +
						'<input id="coHouseSituationList' + _len + '.solve" name="coHouseSituationList[' + _len + '].solve" class="solve " type="text" >' +
						'</td>' +
						'<td class="bg-white">' +
						'<input id="coHouseSituationList' + _len + '.remarks" name="coHouseSituationList[' + _len + '].remarks" class="remarks" type="text" >' +
						'</td>' +
...
//						console.debug("i = "+i);
					$(this).children('td').children('.id').attr('name', 'coHouseSituationList[' + i + '].id');
					$(this).children('td').children('.mainId').attr('name', 'coHouseSituationList[' + i + '].mainId');
					$(this).children('td').children('.idx').attr('name', 'coHouseSituationList[' + i + '].idx');
					$(this).children('td').children('.natureId').attr('name', 'coHouseSituationList[' + i + '].natureId');
					$(this).children('td').children('.housename').attr('name', 'coHouseSituationList[' + i + '].housename');
					$(this).children('td').children('.familyNumber').attr('name', 'coHouseSituationList[' + i + '].familyNumber');
					$(this).children('td').children('.reason').attr('name', 'coHouseSituationList[' + i + '].reason');
					$(this).children('td').children('.solve').attr('name', 'coHouseSituationList[' + i + '].solve');
					$(this).children('td').children('.remarks').attr('name', 'coHouseSituationList[' + i + '].remarks');
					i++;
				});
			};
		});
