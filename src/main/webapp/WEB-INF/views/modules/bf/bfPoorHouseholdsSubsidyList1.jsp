...
//					console.debug("1"+v);
					if (v == "ok") {
						$("#searchForm").attr("action", "${ctx}/bf/bfPoorHouseholdsSubsidy/exportNormal?year=${bfPoorHouseholdsSubsidy.years}");
						$("#searchForm").submit();
					}
				});
			});
			...
			$('#btnSubmit').click(function () {
				$("#searchForm").attr("action", "${ctx}/bf/bfPoorHouseholdsSubsidy/list1");
				page( 1 ,30);
			});
...
<div id="importBox" class="hide">
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bf/bfPoorHouseholdsSubsidy/list1">贫困户补贴列表</a></li>
		...
		<c:if test="${years != null}">
			<c:forEach items="${years}" var="year">
				<c:if test="${yearNow == year}">
					<li class="active"><a href="${ctx}/bf/bfPoorHouseholdsSubsidy/list1?years=${year}">${year}</a></li>
				</c:if>
				<c:if test="${yearNow != year}">
					<li><a href="${ctx}/bf/bfPoorHouseholdsSubsidy/list1?years=${year}">${year}</a></li>
				</c:if>
			</c:forEach>
		</c:if>
	</ul>
